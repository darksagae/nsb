...
export async function queueGenerateInvoice(
  ...
  if (!desktopOnline) {
    ...
    // that can be torn down as soon as it responds, and background work would
    // be killed with it — leaving the invoice on "pending" forever, which is
    // exactly what it is meant to prevent.
    await maybeServerFallbackForUser(userId, trimmed, finalize).catch((err) => {
